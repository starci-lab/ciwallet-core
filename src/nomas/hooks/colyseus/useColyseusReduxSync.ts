...
export const useColyseusReduxSync = (): void => {
    ...
    useEffect(() => {
        const handlePlayerSync = (message: PlayerStateSyncMessage | any) => {
            console.log("🔄 [useColyseusReduxSync] Player state sync received:", message)
            ...
            if (
                ...
                console.log("📦 [useColyseusReduxSync] Extracted player data from response:", payload)
                ...
                console.log("📦 [useColyseusReduxSync] Extracted payload from data:", payload)
            } else {
                // Flat message format
                payload = message
                console.log("📦 [useColyseusReduxSync] Using flat message format:", payload)
            }
            ...
            if (payload?.inventory) {
                console.log("📦 [useColyseusReduxSync] Inventory synced (handled by events)")
            }

            if (payload?.playerData) {
                console.log("📊 [useColyseusReduxSync] Player data synced (handled by events)")
            }
        }

        console.log(`🎧 [useColyseusReduxSync] Listening to event: ${ColyseusMessageEvents.PlayerStateSync}`)
        eventBus.on(ColyseusMessageEvents.PlayerStateSync, handlePlayerSync)

        return () => {
            console.log(`🔇 [useColyseusReduxSync] Removing listener: ${ColyseusMessageEvents.PlayerStateSync}`)
    ...
    useEffect(() => {
        const handleBuyPetResponse = (message: BuyPetResponseMessage) => {
            console.log("🔄 [useColyseusReduxSync] Buy pet response received:", message)
            ...
            if (tokens !== undefined) {
                console.log(`💰 [useColyseusReduxSync] Updating tokens from buy_pet_response: ${tokens}`)
                dispatch(setNomToken(tokens))
            } else {
                console.warn("⚠️ [useColyseusReduxSync] No tokens found in buy_pet_response:", message)
            }
    ...
    useEffect(() => {
        const handlePurchaseResponse = (message: PurchaseResponse) => {
            if (message.newTokenBalance !== undefined) {
                ...
                console.warn("⚠️ [useColyseusReduxSync] No tokens found in purchase_response:", message)
            }
        }
    ...
    useEffect(() => {
        const handleCleanedPetResponse = (message: PurchaseCleanedPetResponse) => {
            console.log("🔄 [useColyseusReduxSync] Cleaned pet response received:", message)
