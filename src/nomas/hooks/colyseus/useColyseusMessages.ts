...
export const useColyseusMessages = (room: Room<GameRoomState> | null): void => {
  useEffect(() => {
     ...
     */
    const handleMessage = (type: string, message: unknown) => {
      console.log(`📨 [useColyseusMessages] Received: ${type}`, message)

      // Map message type to event name
      const eventName = MESSAGE_TO_EVENT_MAP[type] || `colyseus:message:${type}`
