...
export const InitWithdraw = () => {
  ...
  const accountManager = useAppSelector(
    (state) => state.persist.session.accounts[withdrawFormik.values.chainId]
  )

  const withdrawChainId = useAppSelector(
    (state) => state.stateless.withdraw.chainId
  )
  const tokenManager = useAppSelector(
    (state) => state.stateless.withdraw.tokenManager
  )
  const chainManager = useAppSelector(
    (state) => state.stateless.withdraw.chainManager
  )
        ...
        <NomasCardBody>
          ...
          <SelectChainTab
            ...
            onSelect={(chainId) => {
              ...
              dispatch(setChainIdWithdraw(chainId))
          ...
          <div className="flex items-center justify-between mt-4 my-2">
            ...
            <div className="flex items-center gap-2">
              <div className="text-foreground-100">
                Balance:{" "}
                <span>
                  {isBalanceLoading || !(withdrawFormik.values.balance >= 0) ? (
                    <NomasSpinner />
                  ) : (
