...
export class GameScene extends Phaser.Scene {
    ...
    private initializeSystems() {
        ...
        this.time.delayedCall(1000, () => {
            if (colyseusService.isConnected()) {
                ...
                console.log("⏳ [GameScene] Waiting for connection before requesting pets state")
    ...
    private setupColyseusPhaserFeatures() {
        ...
        const handleError = (event: ColyseusErrorEvent) => {
            ...
            if (this.gameUI && this.lastClickPosition) {
                this.gameUI.showNotification(`${event.message}`, this.lastClickPosition.x, this.lastClickPosition.y)
            }
        }
        ...
        const handlePurchaseError = (message: { success: boolean; message: string }) => {
            if (!message.success && this.gameUI && this.lastClickPosition) {
                this.gameUI.showNotification(`${message.message}`, this.lastClickPosition.x, this.lastClickPosition.y)
            }
        }

        const handleBuyPetError = (message: { success: boolean; message: string }) => {
            if (!message.success && this.gameUI && this.lastClickPosition) {
                this.gameUI.showNotification(`${message.message}`, this.lastClickPosition.x, this.lastClickPosition.y)
            }
        }
        ...
        this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => {
            ...
            eventBus.off(ColyseusMessageEvents.PurchaseItemResponse, handlePurchaseError)
    ...
    private showConnectionStatus(text: string, color: string = "#ff0000") {
        ...
        this.connectionStatusText = this.add.text(10, 70, text).setStyle({ color, fontSize: "12px" }).setPadding(4)
    ...
    attachColyseusRoom(_room: unknown) {
        console.log("⚠️ [GameScene] attachColyseusRoom is deprecated - room is managed by ColyseusProvider")
    ...
    async connectToColyseus(_url: string = envConfig().colyseus.endpoint) {
        console.log("⚠️ [GameScene] connectToColyseus is deprecated - connection is handled by ColyseusProvider")
    ...
    sendBuyFoodLegacy(payload: { itemType: string; itemName: string; quantity: number; itemId: string }) {
        if (!colyseusService.isConnected()) return
        colyseusService.purchaseItem(payload.itemType, payload.itemName, payload.quantity, payload.itemId)
    }
