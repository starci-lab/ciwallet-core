...
export class InputManager {
    ...
    constructor(scene: GameScene, petManager: PetManager, notificationUI: NotificationUI) {
        this.scene = scene
        ...
        this.scene.input.on("pointerdown", (pointer: Phaser.Input.Pointer) => {
            ...
            if (placing && placing.type === "food") {
                if (isDoubleClick) {
                    ...
                    this.notificationUI.showNotification("Canceled placement", pointer.x, pointer.y)
            ...
            if (placing && placing.type === "clean") {
                ...
                const purchased = this.petManager.buyAndCleanPoop(pointer.x, pointer.y, placing.itemId)
                if (!purchased) {
                    ...
                    this.notificationUI.showNotification("No poop found at this location", pointer.x, pointer.y)
                }
    ...
    private handlePetInteraction(x: number, y: number) {
        ...
        const distance = Phaser.Math.Distance.Between(x, y, petBounds.centerX, petBounds.centerY)
        ...
        if (distance < 100) {
            ...
            this.notificationUI.showNotification(`Pet interaction: ${randomInteraction}`, x, y)
