...
export class PetManager {
    ...
    createPet(petId: string, x: number, y: number, petType: string = "chog"): PetData {
        ...
        const cleanlinessSystem = new CleanlinessSystem(this.scene, pet, colyseusService, petId)
    ...
    addSharedFoodFromServer(foodId: string, serverFood: any): void {
        ...
        const foodSprite = this.scene.add.sprite(serverFood.x, serverFood.y, "hamburger")
        ...
        // Create shadow
        const shadow = this.scene.add.ellipse(serverFood.x, serverFood.y + 10, 30, 15, 0x000000, 0.3)
    ...
    private handleFoodRemovalForPets(removedFood: Phaser.GameObjects.Sprite): void {
        for (const petData of this.pets.values()) {
            if (petData.pet.chaseTarget && petData.pet.chaseTarget === removedFood) {
                console.log(`🚶 Pet ${petData.id} target food removed, returning to walk`)
                petData.pet.stopChasing()
    ...
    update(): void {
        for (const petData of this.pets.values()) {
            ...
            this.checkSharedBallPlaying(petData, petData.pet.sprite.x, petData.pet.sprite.y)
    ...
    buyAndCleanPoop(x: number, y: number, cleaningId: string = "broom"): boolean {
        ...
        for (const petData of Object.values(allPetsData)) {
            if (!petData.cleanlinessSystem || petData.cleanlinessSystem.poopObjects.length === 0) {
                continue
            }
    ...
    private async dropSharedFood(x: number, _y?: number, foodId: string = "hamburger"): Promise<void> {
        ...
        // Add shadow effect
        const shadow = this.scene.add.ellipse(clampedX, foodFinalY + 8, 30, 12, 0x000000, 0.3)
        this.scene.tweens.add({
            targets: shadow,
            scaleX: 1.3,
            alpha: 0.5,
    ...
    private removeSharedBallAtIndex(index: number): void {
        ...
        if (chasingPetId) {
            ...
            if (chasingPetData && chasingPetData.pet.isChasing && chasingPetData.pet.chaseTarget) {
        ...
        if (wasBeingChased && chasingPetData) {
            console.log(`🎾 Pet ${chasingPetData.id} was chasing ball that disappeared, handling gracefully`)

            // Stop chasing immediately and trigger the play action
            chasingPetData.pet.stopChasing()
            chasingPetData.happinessSystem.triggerPlay(GAME_MECHANICS.HAPPINESS_INCREASE_AMOUNT)

            // Quick transition to avoid stuttering
            this.scene.time.delayedCall(30, () => {
                if (chasingPetData.happinessSystem.happinessLevel < 100 && this.sharedDroppedBalls.length > 0) {
                    console.log(`🔄 Pet ${chasingPetData.id} looking for another ball after playing`)
                    this.checkPetShouldChaseBalls(chasingPetData)
                } else {
                    console.log(`🚶 Pet ${chasingPetData.id} returning to walk mode after playing`)
    ...
    private async dropSharedToy(x: number, _y?: number, toyId: string = "ball"): Promise<void> {
        ...
        if (!this.scene.textures.exists(textureKey)) {
            ...
            this.sharedBallShadows.push(this.scene.add.ellipse(clampedX, toyFinalY + 15, 20, 8, 0x000000, 0.3))
            return
        }
        ...
        // Add shadow effect
        const shadow = this.scene.add.ellipse(clampedX, toyFinalY + 15, 20, 8, 0x000000, 0.3)
        this.scene.tweens.add({
            targets: shadow,
    ...
    private dropSharedBall(x: number, _y?: number): void {
        ...
        // Add shadow effect
        const shadow = this.scene.add.ellipse(clampedX, ballFinalY + 15, 20, 8, 0x000000, 0.3)
        this.scene.tweens.add({
            targets: shadow,
            scaleX: { from: 0.5, to: 1 },
            scaleY: { from: 0.5, to: 1 },
    ...
    checkSharedBallPlaying(petData: PetData, x: number, y: number): boolean {
        ...
        if (ballIndex !== -1) {
            console.log(`🎾 Pet ${petData.id} is playing with ball at index ${ballIndex}`)
            ...
            petData.pet.stopChasing()
            petData.happinessSystem.triggerPlay(GAME_MECHANICS.HAPPINESS_INCREASE_AMOUNT)
    ...
    private handlePetPostEating(petData: PetData): void {
        ...
        this.scene.time.delayedCall(3000, () => {
            ...
            if (petData.pet.currentActivity === "chew" || petData.pet.isUserControlled) {
                // Check if pet should continue chasing more food or return to auto walk
                if (petData.feedingSystem.hungerLevel < 100 && this.sharedDroppedFood.length > 0) {
    ...
    private forceReturnToWalk(petData: PetData): void {
        ...
        this.scene.time.delayedCall(500, () => {
            if (petData.pet.isUserControlled || petData.pet.currentActivity !== "walk") {
                petData.pet.isUserControlled = false
                petData.pet.isChasing = false
                petData.pet.chaseTarget = null
    ...
    debugPetsStatus(): void {
        ...
        for (const petData of this.pets.values()) {
            ...
            console.log(`  Position: (${petData.pet.sprite.x.toFixed(1)}, ${petData.pet.sprite.y.toFixed(1)})`)
            ...
            console.log(
                `  In Bounds: ${petData.pet.sprite.x >= petBounds.minX && petData.pet.sprite.x <= petBounds.maxX}`
            )
            console.log(
                `  Chase Target: ${
                    petData.pet.chaseTarget ? `(${petData.pet.chaseTarget.x}, ${petData.pet.chaseTarget.y})` : "None"
                }`
            )
        }
    ...
    handlePetRightClick(petId: string): void {
        ...
        //   this.scene.gameUI.petDetailsModal.showForPet(petData)
        // } else {
    ...
    public updateAllScales(): void {
        ...
        // Update pets
        const responsivePetScale = GamePositioning.getResponsivePetScale(cameraWidth)
        ...
        // Update food items
        const responsiveFoodScale = GamePositioning.getResponsiveFoodScale(cameraWidth)
