...
export class ColyseusService {
    ...
    initialize(): void {
        if (this.initialized) {
            console.warn("[ColyseusService] Already initialized")
            return
        }
        ...
        eventBus.on(ColyseusActionEvents.PurchaseItem, (action: PurchaseItemAction) => {
        ...
        eventBus.on(ColyseusActionEvents.PlayWithPet, (action: PlayWithPetAction) => {
        ...
        eventBus.on(ColyseusActionEvents.RequestPlayerState, (_action: RequestPlayerStateAction) => {
            this.sendMessage("request_player_state", {})
        })

        eventBus.on(ColyseusActionEvents.RequestPetsState, (_action: RequestPetsStateAction) => {
            this.sendMessage("request_pets_state", {})
        })

        eventBus.on(ColyseusActionEvents.GetStoreCatalog, (_action: GetStoreCatalogAction) => {
            this.sendMessage("get_store_catalog", {})
        })

        eventBus.on(ColyseusActionEvents.GetInventory, (_action: GetInventoryAction) => {
            this.sendMessage("get_inventory", {})
        })

        eventBus.on(ColyseusActionEvents.CancelPurchase, (action: CancelPurchaseAction) => {
    ...
    sendMessage(type: string, data: unknown): void {
        if (!this.room) {
            console.warn("⚠️ [ColyseusService] Cannot send message - room is null", type, data)
            return
        }

        if (!this.isConnected()) {
            console.warn("⚠️ [ColyseusService] Cannot send message - not connected", type, data)
            return
        }
     ...
     */
    purchaseItem(itemType: string, itemName: string, quantity: number = 1, itemId: string): void {
     ...
     */
    purchaseItemV2(purchaseId: string, itemType: string, itemId: string, quantity: number, price: number): void {
     ...
     */
    eatedFood(data: { hunger_level: number; pet_id: string; owner_id: string }): void {
     ...
     */
    cleanedPet(data: { cleanliness_level: number; pet_id: string; owner_id: string }): void {
     ...
     * Handle pet playing
     */
    playedPet(data: { happiness_level: number; pet_id: string; owner_id: string }): void {
     ...
     */
    createPoop(data: { petId: string; positionX: number; positionY: number }): void {
